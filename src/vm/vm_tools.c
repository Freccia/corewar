...
/*   Updated: 2018/03/17 19:45:38 by nfinkel          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
				...
				t_flag flags)
{
	...
	if (flags == E_DIR)
		...
		pos = cw_mem_inc(ocp, ft_mtoi(cw_map_mem(mem, *ptr), len) % IDX_MOD);
	else
		pos = cw_mem_inc(ocp, ft_mtoi(cw_map_mem(mem, *ptr), len));
	*ptr = cw_mem_inc(*ptr, len);

...
/*	uint8_t		*ocp; //
	...
	ocp = cw_mem_inc(pc, 1);
	if (!(((*ocp & 0xc0) >> 6) & g_op_tab[*pc - 1].param_type[0]))
		return (EXIT_FAILURE);
	if (!(((*ocp & 0x30) >> 4) & g_op_tab[*pc - 1].param_type[1]))
		return (EXIT_FAILURE);
	if (!(((*ocp & 0x0c) >> 2) & g_op_tab[*pc - 1].param_type[2]))
		return (EXIT_FAILURE);*/
	(void)pc;
	return (EXIT_SUCCESS);
}
...
int		cw_vm_exec(uint8_t *pc)
{
	...
	k = -1;
	while (++k < MAX_OP)
		if (*pc == g_op_tab[k].op_code)
			if (!g_op_tab[k].ocp || cw_check_ocp(pc) == EXIT_SUCCESS)
